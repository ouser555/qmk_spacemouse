...
#define	AZOTEQ_IQS5XX_REG_SWIPETIME	    0x06BF	//(READ/WRITE/E2)	//2 BYTES; 
#define	AZOTEQ_IQS5XX_REG_SWIPEDIST	    0x06C1	//(READ/WRITE/E2)	//2 BYTES; 
#define	AZOTEQ_IQS5XX_REG_SWIPCONSTIME  0x06C2	//(READ/WRITE/E2)	//2 BYTES; 
#define	AZOTEQ_IQS5XX_REG_SWIPECONSDIST	0x06C5	//(READ/WRITE/E2)	//2 BYTES; 
#define	AZOTEQ_IQS5XX_REG_SWIPEANGLE    0x06C7	//(READ/WRITE/E2)
#define	AZOTEQ_IQS5XX_REG_SCROLLDIST    0x06C8	//(READ/WRITE/E2)	//2 BYTES; 
#define	AZOTEQ_IQS5XX_REG_SCROLLANGLE   0x06CA	//(READ/WRITE/E2)
#define	AZOTEQ_IQS5XX_REG_ZOOMDIST      0x06CB	//(READ/WRITE/E2)	//2 BYTES; 
#define	AZOTEQ_IQS5XX_REG_ZOOMCONDIST	0x06CD	//(READ/WRITE/E2)	//2 BYTES; 
...
i2c_status_t ChannelSetupPXM0002(void) {
    ...
    //uint8_t xReso[2]={0x08,0x19}; // 2304/10x9=2073
    //uint8_t yReso[2]={0x0E,0x00}; // 3584

    uint8_t xReso[2]={0x03,0x00}; //768
    uint8_t yReso[2]={0x04,0x00}; //1024

    //uint8_t xReso[2]={0x01,0xF0}; //496
    //uint8_t yReso[2]={0x02,0x00}; //512 
    ...
    //uint8_t touchSet = 12;
    //uint8_t touchClear = 8;
    uint8_t touchSet = 7;
    uint8_t touchClear = 3;
    ...
    //uint8_t swipeInitTime[2] = {0x00,0x96}; // 150ms
    uint8_t swipeInitTime[2] = {0x00,0xC8}; // 200ms
    //uint8_t swipeInitDist[2] = {0x00,0x5A}; // 90pix
    uint8_t swipeInitDist[2] = {0x00,0x32}; // 50pix
 
    //uint8_t swipeInitTime[2] = {0x01,0xF4}; // 500ms
    //uint8_t swipeInitDist[2] = {0x00,0x1E}; // 30
    uint8_t swipeConsTime[2] = {0,0}; // 0
    //uint8_t swipeConsDist[2] = {0x00,0x96}; // 150
    
    uint8_t scrollInitDist[2] = {0x00,0x08}; //8
#endif
#if 0
    //uint8_t swipeInitTime[2] = {0x00,0x64}; // 100ms
    //uint8_t swipeInitDist[2] = {0x07,0xD0}; // 200pix
    uint8_t swipeConsDist[2] = {0x00,0x96}; // 150
#endif

    //uint8_t swipeAngle = 37; // 30deg
    uint8_t swipeAngle = 61; // 44deg
    ...
    status |= i2c_writeReg16(AZOTEQ_IQS5XX_ADDRESS, AZOTEQ_IQS5XX_REG_SWIPCONSTIME, (uint8_t *)&swipeConsTime, 2, AZOTEQ_IQS5XX_TIMEOUT_MS);
    //status |= i2c_writeReg16(AZOTEQ_IQS5XX_ADDRESS, AZOTEQ_IQS5XX_REG_SWIPECONSDIST, (uint8_t *)&swipeConsDist, 2, AZOTEQ_IQS5XX_TIMEOUT_MS);
    status |= i2c_writeReg16(AZOTEQ_IQS5XX_ADDRESS, AZOTEQ_IQS5XX_REG_SWIPETIME, (uint8_t *)&swipeInitTime, 2, AZOTEQ_IQS5XX_TIMEOUT_MS);
    status |= i2c_writeReg16(AZOTEQ_IQS5XX_ADDRESS, AZOTEQ_IQS5XX_REG_SWIPEDIST, (uint8_t *)&swipeInitDist, 2, AZOTEQ_IQS5XX_TIMEOUT_MS);
        
    status |= i2c_writeReg16(AZOTEQ_IQS5XX_ADDRESS, AZOTEQ_IQS5XX_REG_SCROLLDIST, (uint8_t *)&scrollInitDist, 2, AZOTEQ_IQS5XX_TIMEOUT_MS);
#endif
#if 0
    //status |= i2c_writeReg16(AZOTEQ_IQS5XX_ADDRESS, AZOTEQ_IQS5XX_REG_SWIPETIME, (uint8_t *)&swipeInitTime, 2, AZOTEQ_IQS5XX_TIMEOUT_MS);
    //status |= i2c_writeReg16(AZOTEQ_IQS5XX_ADDRESS, AZOTEQ_IQS5XX_REG_SWIPEDIST, (uint8_t *)&swipeInitDist, 2, AZOTEQ_IQS5XX_TIMEOUT_MS);
    status |= i2c_writeReg16(AZOTEQ_IQS5XX_ADDRESS, AZOTEQ_IQS5XX_REG_SWIPECONSDIST, (uint8_t *)&swipeConsDist, 2, AZOTEQ_IQS5XX_TIMEOUT_MS);
#endif
    status |= i2c_writeReg16(AZOTEQ_IQS5XX_ADDRESS, AZOTEQ_IQS5XX_REG_SWIPEANGLE, (uint8_t *)&swipeAngle, 1, AZOTEQ_IQS5XX_TIMEOUT_MS);
