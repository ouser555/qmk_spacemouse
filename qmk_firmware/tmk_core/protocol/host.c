...
void host_3Dmouse1_send(report_3dmouse1_t *report) {
    ...
    if (!driver) return;
#ifdef MOUSE_SHARED_EP
    report->report_id = REPORT_ID_3DMOUSE1;
#endif
#if 0
    report->x = report->x;
    report->y = report->y;
    report->z = report->z;
#endif
    //(*driver->send_3dmouse1)(report);
    send_3dmouse1(report);
}
__attribute__((weak)) void send_3dmouse1(report_3dmouse1_t *report) {}
//--------------------------------------------------------------------
void host_3Dmouse2_send(report_3dmouse2_t *report) {

    if (!driver) return;
#ifdef MOUSE_SHARED_EP
    report->report_id = REPORT_ID_3DMOUSE2;
#endif
#if 0
    report->r = report->r;
    report->p = report->p;
    report->y = report->y;
#endif
    //(*driver->send_3dmouse2)(report);
    send_3dmouse2(report);
}

__attribute__((weak)) void send_3dmouse2(report_3dmouse2_t *report) {}
